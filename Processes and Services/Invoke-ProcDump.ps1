<# 
.SYNOPSIS:
        Dumps the processes of the McAfee Agent, compresses each one, copies the files to the local machine, and uncompresses them. The .dmp file can then be analyzed using your
        favorite analysis tool.

.PARAMETER dumpproc
    Used to specify which process to dump.

.EXAMPLE
    PS C:\> .\Invoke-ProcDump.ps1 -DumpProc notepad

    Dumps notepad process. 
    
#>


param(
[Parameter(Mandatory=$true)][string]$DumpProc
)
...
"function DumpProc" >> .\proc_dump.ps1
"{" >> .\proc_dump.ps1
"[CmdletBinding()]" >> .\proc_dump.ps1
"    Param (" >> .\proc_dump.ps1
"        [Parameter(Position = 0, Mandatory = `$True, ValueFromPipeline = `$True)]" >> .\proc_dump.ps1
"        [System.Diagnostics.Process]" >> .\proc_dump.ps1
"        `$Process," >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"        [Parameter(Position = 1)]" >> .\proc_dump.ps1
"        [ValidateScript({ Test-Path `$_ })]" >> .\proc_dump.ps1
"        [String]" >> .\proc_dump.ps1
"        `$DumpFilePath = 'c:\'" >> .\proc_dump.ps1
"    )" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"    BEGIN" >> .\proc_dump.ps1
"    {" >> .\proc_dump.ps1
"        `$WER = [PSObject].Assembly.GetType('System.Management.Automation.WindowsErrorReporting')" >> .\proc_dump.ps1
"        `$WERNativeMethods = `$WER.GetNestedType('NativeMethods', 'NonPublic')" >> .\proc_dump.ps1
"        `$Flags = [Reflection.BindingFlags] 'NonPublic, Static'" >> .\proc_dump.ps1
"        `$MiniDumpWriteDump = `$WERNativeMethods.GetMethod('MiniDumpWriteDump', `$Flags)" >> .\proc_dump.ps1
"        `$MiniDumpWithFullMemory = [UInt32] 2" >> .\proc_dump.ps1
"    }" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"    PROCESS" >> .\proc_dump.ps1
"    {" >> .\proc_dump.ps1
"        `$ProcessId = `$Process.Id" >> .\proc_dump.ps1
"        `$ProcessName = `$Process.Name" >> .\proc_dump.ps1
"        `$ProcessHandle = `$Process.Handle" >> .\proc_dump.ps1
"        `$ProcessFileName = '$($ProcessName).dmp'" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"        `$ProcessDumpPath = Join-Path `$DumpFilePath `$ProcessFileName" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"        `$FileStream = New-Object IO.FileStream(`$ProcessDumpPath, [IO.FileMode]::Create)" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"        `$Result = `$MiniDumpWriteDump.Invoke(`$null, @(`$ProcessHandle," >> .\proc_dump.ps1
"                                                     `$ProcessId," >> .\proc_dump.ps1
"                                                     `$FileStream.SafeFileHandle," >> .\proc_dump.ps1
"                                                     `$MiniDumpWithFullMemory," >> .\proc_dump.ps1
"                                                     [IntPtr]::Zero," >> .\proc_dump.ps1
"                                                     [IntPtr]::Zero," >> .\proc_dump.ps1
"                                                     [IntPtr]::Zero))" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"        `$FileStream.Close()" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"        if (-not `$Result)" >> .\proc_dump.ps1
"        {" >> .\proc_dump.ps1
"            `$Exception = New-Object ComponentModel.Win32Exception" >> .\proc_dump.ps1
"            `$ExceptionMessage = '$($Exception.Message) ($($ProcessName):$($ProcessId))' " >> .\proc_dump.ps1
"            # Remove any partially written dump files. For example, a partial dump will be written" >> .\proc_dump.ps1
"            # in the case when 32-bit PowerShell tries to dump a 64-bit process." >> .\proc_dump.ps1
"            Remove-Item `$ProcessDumpPath -ErrorAction SilentlyContinue" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"            throw `$ExceptionMessage" >> .\proc_dump.ps1
"        }" >> .\proc_dump.ps1
"        else" >> .\proc_dump.ps1
"        {" >> .\proc_dump.ps1
"        }" >> .\proc_dump.ps1
...
"}" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"`$procs = '$DumpProc'" >> .\proc_dump.ps1
"" >> .\proc_dump.ps1
"foreach(`$my_proc in `$procs) " >> .\proc_dump.ps1
"    { " >> .\proc_dump.ps1
"    get-process `$my_proc | dumpproc " >> .\proc_dump.ps1
"    makecab c:\.dmp c:\`$my_proc.cab  >`$null 2>&1 " >> .\proc_dump.ps1
"    remove-item c:\.dmp " >> .\proc_dump.ps1
"    } " >> .\proc_dump.ps1
...
foreach($computer in $computers)
    {
    copy-item proc_dump.ps1 \\$computer\C$\.
    Invoke-WmiMethod -Class Win32_Process -Name Create -Computername $computer -ArgumentList "powershell.exe /c c:\proc_dump.ps1 " >$null 2>&1
...
foreach($computer in $computers)
    {
    copy-item \\$computer\c$\*cab .\
    expand .\$DumpProc.cab .\$computer-$dumpProc.dmp
    Write-Host "Data pulled back from $computer" -ForegroundColor cyan
    remove-item \\$computer\c$\*.dmp
    remove-item \\$computer\c$\*.cab
    remove-item \\$computer\c$\proc_dump.ps1
    remove-item .\$dumpproc.cab
    }

Remove-Item .\proc_dump.ps1
