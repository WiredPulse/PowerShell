...
.PARAMETER ExcludeSimilar
    This will filter out similar programnames, the default value is to filter on the first 3 words in a program name. If a program only consists of less words it is 
    excluded and it will not be filtered. For example if you Visual Studio 2015 installed it will list all the components individually, using -ExcludeSimilar will only 
...
.EXAMPLE
    Get-SoftwareInstallData

    Description:
    Will generate a list of installed programs on local machine

.EXAMPLE
    Get-SoftwareInstallData -ComputerName server01,server02

    Description:
    Will generate a list of installed programs on server01 and server02

.EXAMPLE
    Get-SoftwareInstallData -ComputerName Server01 -Property DisplayVersion,VersionMajor
...
.EXAMPLE
    'server01','server02' | Get-SoftwareInstallData -Property Uninstallstring
...
.EXAMPLE
    'server01','server02' | Get-SoftwareInstallData -Property Uninstallstring -ExcludeSimilar -SimilarWord 4

    Description
    Will retrieve the installed programs on server01/02 that are passed on to the function through the pipeline and also retrieves the uninstall string for each program. Will 
    only display a single entry of a program of which the first four words are identical.

.EXAMPLE
    'server01','server02' | Get-SoftwareInstallData -Property installdate

    Description
    Will retrieve the installed programs on server01/02 that are passed on to the function through the pipeline and also retrieves the install date string for each program
