<# 
    .SYNOPSIS:
        Retrieves the McAfee Agent running processes along with their DLLs. The file size, memory size, publishing company, and file location is also retrieve.The four processes
        that the McAfee Agent uses is naPrdMgr.exe, UdaterUI.exe, McTray.exe, and FrameworkService.exe.

    .USAGE:
        1. Change the variables in line 29 and 30
        ...
        4. Read results back into PowerShell using this syntax: import-csv procmods.csv | out-gridview
    ...
    .NOTES  
        File Name      : Get-ProcMod_Initiator.ps1
        Version        : v.0.1  
...
foreach($computer in $computers)
    ...
    Invoke-WmiMethod -Class Win32_Process -Name Create -Computername $computer -ArgumentList "powershell.exe /c c:\$script2run | export-csv c:\$computer.txt" >$null 2>&1
...
foreach($computer in $computers)
    ...
    remove-item .\$computer'_'.txt
    remove-item \\$computer\c$\$computer.txt
    remove-item \\$computer\c$\get-procmods.ps1
    }
...
Import-csv ".\proc.csv" -Delimiter ',' -Header 'System', 'Size', 'Modulename', 'Filename', 'Company', 'ModuleMemorySize' | export-csv .\ProcMods.csv
