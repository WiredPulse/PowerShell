...
.PARAMETER Path2PROC
    Used to specify the path to the running processes executable.

.EXAMPLE
    PS C:\> .\Invoke-ProcessStrings.ps1 -Path2EXE 'c:\users\blue\desktop\strings.exe' -Path2Proc 'C:\Program Files (x86)\nxlog\nxlog.exe'

    Specifies the location of 'strings.exe' and to run strings against 'nxlog.exe'. 
...
[Parameter(Mandatory=$true)][string]$Path2Proc
)
...
"c:\strings.exe -accepteula '$path2proc' | out-file c:\99099_proc.txt " >> .\Get-Strings.ps1
...
# Pulling data back
# ==============================================================================
$my_proc = $path2proc.split('\') | select -last 1

foreach($computer in $computers)
    {
    copy-item \\$computer\c$\99099_*.txt .\
    $proc = get-content .\99099_*.txt
    $proc2 = $proc | foreach {'"'+ $computer +'",'+ $my_proc +',' + $_} | select -skip 5 >> strings2.csv
    Write-Host "Data pulled back from $computer" -ForegroundColor cyan
    remove-variable proc, proc2
...
# ==============================================================================
Import-csv ".\strings2.csv" -Delimiter ',' -Header 'System', 'Executable', 'Strings' | export-csv .\Strings.csv
remove-item .\strings2.csv
