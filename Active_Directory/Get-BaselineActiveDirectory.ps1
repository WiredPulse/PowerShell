...
.USAGE
    To get the initial baseline, run the script and select option 1 to get a baseline of accounts OR slect option 2 to compare account with baselines. 

    If option 1 is selected, a directory called 'AD_Check' will be created at 'c:\users\$env:USERNAME\desktop\'. In the directory a 'Baseline_Admins.txt' 
    and 'Baseline_AD_Users.txt' will be created. A '_Domain_Changes_Log.txt' file will be created in the folder as well.

    If option 2 is selected, a 'Baseline_Admins_Compare.txt' and 'Baseline_Users_Compare.txt' will be created in the 'AD_Check' folder located at 
    'Set-Location c:\users\$env:USERNAME\desktop\AD_Check'. The new .txt will be compare with the old .txt. Changes will be noted in the '_Domain_Changes_Log.txt' 
    file as well.
...
$answer = read-host "Please Make a Selection"   
    if ($answer -eq 1)
        ...
        New-item c:\users\$env:USERNAME\desktop\AD_Check -ItemType directory | Out-Null
        Set-Location c:\users\$env:USERNAME\desktop\AD_Check
