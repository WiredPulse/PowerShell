...
.EXAMPLE
    PS C:\> Get-RegValueDataList.ps1 -HKEY_LOCAL_MACHINE -key SOFTWARE\Microsoft\Windows NT\CurrentVersion -value CurrentBuild

    Returns the data listed 'CurrentBuild' Value within the 'HKLM:\CurrentVersion\SOFTWARE\Microsoft\Windows NT\CurrentVersion' Key
...
foreach($cpu in $computers){
    ...
    $r = $reg.GetStringValue($hive, $key, $value).svalue  ## REG_SZ
...
# ==============================================================================
import-csv "c:\users\$current_user\desktop\Reg_value.txt" -Delimiter '+' -Header 'System', 'Key' | export-csv c:\users\$current_user\desktop\Registry_Value.csv
