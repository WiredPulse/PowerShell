<#
SYNOPSIS:
    Queries a specific Registry Key and returns the Sub-Keys of it. The returned data is in the form of a xml which can be read back in
    using "import-csv .\registry_Keys.xml | out-gridview ".

USAGE:
    1) Adjust the variables in lines 39 and 42
    2) Run the script with an account with WMI rights and follow the prompts
    3) A file called "registry_keys.xml"... read it back into PowerShell using the follosing syntax:  import-csv .\registry_keys.xml | out-gridview 

    .NOTES  
        File Name      : Get-RegKeysList.ps1
        Version        : v.0.1  
        Author         : @WiredPulse
...
# ==============================================================================
import-csv "c:\users\$current_user\desktop\Reg.txt" -Delimiter ' ' -Header 'System', 'Key' |export-csv c:\users\$current_user\desktop\Registry_Keys.xml
